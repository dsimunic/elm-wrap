...
static void print_diff_usage(void) {
    printf("Usage: %s diff [VERSION]\n", program_name);
    printf("\n");
    printf("Show API differences between package versions.\n");
    printf("\n");
    printf("This command ensures all package dependencies are downloaded and cached\n");
    printf("before calling 'elm diff'.\n");
    printf("\n");
    printf("All options are passed through to 'elm diff'.\n");
}
...
int cmd_diff(int argc, char *argv[]) {
    ...
    // Now call elm diff with all the arguments
    printf("\nAll dependencies cached. Running elm diff...\n\n");
    ...
    // Build elm diff command
    // We need to pass all arguments except "diff" to elm
    ...
    // Execute elm diff with custom environment
