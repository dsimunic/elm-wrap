/**
 * init_v2.c - V2 Protocol Implementation for elm init
 *
 * This module handles dependency resolution for `elm init` using the V2 registry.
 */
...
static const char *pg_name_resolver_v2(void *ctx, PgPackageId pkg) {
    PgElmV2Context *pg_ctx = (PgElmV2Context *)ctx;
...
bool solve_init_dependencies_v2(InstallEnv *env, PackageMap **direct_deps, PackageMap **indirect_deps) {
    // Verify V2 registry is available
    if (!env->v2_registry) {
        log_error("V2 protocol active but registry is not loaded");
        return false;
    }

    // Create PubGrub solver context for V2
    PgElmV2Context *pg_ctx = pg_elm_v2_context_new(env->v2_registry);
    if (!pg_ctx) {
        log_error("Failed to create PubGrub solver context (V2)");
        return false;
    }
    ...
    for (int i = 0; i < 3; i++) {
        ...
        PgPackageId pkg_id = pg_elm_v2_intern_package(pg_ctx, author, name);
        if (pkg_id < 0) {
        ...
        if (!pg_elm_v2_add_root_dependency(pg_ctx, pkg_id, range)) {
    ...
    if (status != PG_SOLVER_OK) {
        ...
        char error_msg[4096];
        if (pg_solver_explain_failure(solver, pg_name_resolver_v2, pg_ctx, error_msg, sizeof(error_msg))) {
            fprintf(stderr, "%s\n", error_msg);
        }
    ...
    *indirect_deps = package_map_create();
    ...
    for (int pkg_id = 1; pkg_id < pg_ctx->package_count; pkg_id++) {
        ...
        for (int i = 0; i < 3; i++) {
            const char *author = required_packages[i][0];
            const char *name = required_packages[i][1];
            PgPackageId direct_pkg_id = pg_elm_v2_intern_package(pg_ctx, author, name);
