...
static bool rel_path_is_hidden_from_reporting(const char *rel_path) {
    if (!rel_path || rel_path[0] == '\0') return false;

    /* Keep .gitattributes functionality, but never report or write .git. */
    if (strcmp(rel_path, ".git") == 0) return true;
    if (strncmp(rel_path, ".git/", 5) == 0) return true;

    return false;
}

static void user_print_separator_line(const char *title) {
    if (!title) title = "";

    /* Target an ~80 character line, e.g. "==== EXTRA FILES ==============================" */
    enum { WIDTH = 80 };
    char line[WIDTH + 1];
    size_t pos = 0;
    ...
    size_t remaining = (pos < WIDTH ? (WIDTH - pos) : 0);
    if (title_len > remaining) title_len = remaining;
    if (title_len > 0) {
        memcpy(line + pos, title, title_len);
        pos += title_len;
    }
...
static bool prompt_yes_no_default_yes_line(const char *prompt) {
    ...
    /* Default yes on empty line */
    if (response[0] == '\n' || response[0] == '\0') return true;
    if (response[0] == 'N' || response[0] == 'n') return false;
    if (response[0] == 'Y' || response[0] == 'y') return true;
    return true;
}

static void user_print_gitattributes_preview_for_extras(
    const char *pkg_root,
    ...
    int already_excluded_count
) {
    if (!pkg_root) return;

    if (file_exists_already && already_excluded_count > 0) {
        if (already_excluded_count == 1) {
            user_message("One of which is already excluded from publishing in .gitattributes\n\n");
        } else {
            user_message("%d of which are already excluded from publishing in .gitattributes\n\n",
                         already_excluded_count);
        }
    }

    user_message("Here is my plan:\n\n");

    if (file_exists_already) {
        user_message("  Update the .gitattributes file with the following contents:\n");
    } else {
        user_message("  Write a .gitattributes file with the following contents:\n");
    }

    user_message("    # Generated by wrap package prepublish --git-exclude-extras\n");
    user_message("    # These files and directories are excluded from git archive (e.g., GitHub releases)\n");
    user_message("    \n");
...
static bool string_contains_line(const char *haystack, const char *needle_line) {
    if (!haystack || !needle_line) return false;

    const size_t needle_len = strlen(needle_line);
    if (needle_len == 0) return false;

    const char *p = haystack;
    while ((p = strstr(p, needle_line)) != NULL) {
        const char before = (p == haystack) ? '\n' : p[-1];
        const char after = p[needle_len];

        bool ok_before = (before == '\n' || before == '\r');
        bool ok_after = (after == '\0' || after == '\n' || after == '\r');
...
static int count_existing_gitattributes_exclusions_for_extras(
    const char *pkg_root,
    const char **extra_files,
    int extra_files_count,
    const char **extra_dirs,
    ...
    if (!pkg_root) return 0;

    char gitattributes_path[MAX_PATH_LENGTH];
    snprintf(gitattributes_path, sizeof(gitattributes_path), "%s/.gitattributes", pkg_root);

    struct stat st;
    if (stat(gitattributes_path, &st) != 0) {
        return 0;
    }

    char *content = file_read_contents_bounded(gitattributes_path, MAX_FILE_READ_CONTENTS_BYTES, NULL);
    if (!content) return 0;
    ...
    for (int i = 0; i < extra_files_count; i++) {
        ...
        char needle[MAX_PATH_LENGTH + 32];
        snprintf(needle, sizeof(needle), "/%s export-ignore", rel_path);
        if (string_contains_line(content, needle)) {
            already++;
        }
    }
...
int cmd_package_prepublish(int argc, char *argv[]) {
    ...
    /* This command never deletes package files/extras from disk.
     * It may delete `elm-stuff` as part of the silent compile check. */
    ...
    /* Filter extras into reportable vs hidden-from-reporting.
     * Hidden entries are excluded from both reporting and .gitattributes generation. */
    int report_extra_capacity = extra_count;
    int report_extra_count = 0;
    const char **report_extra_abs = arena_malloc((report_extra_capacity > 0 ? report_extra_capacity : 1) * sizeof(char*));

    for (int i = 0; i < extra_count; i++) {
        if (!extra_abs || !extra_abs[i]) continue;
        const char *rel = (extra_rel ? extra_rel[i] : NULL);
        if (rel && rel_path_is_hidden_from_reporting(rel)) {
            continue;
        }
        report_extra_abs[report_extra_count++] = extra_abs[i];
    }

    int report_skipped_capacity = skipped_dirs_count;
    int report_skipped_count = 0;
    const char **report_skipped_dirs = arena_malloc((report_skipped_capacity > 0 ? report_skipped_capacity : 1) * sizeof(char*));

    size_t clean_abs_len = strlen(clean_abs);
    for (int i = 0; i < skipped_dirs_count; i++) {
        if (!skipped_dirs[i]) continue;
        struct stat st;
        if (stat(skipped_dirs[i], &st) != 0) continue;

        const char *abs_path = skipped_dirs[i];
        const char *rel_path = abs_path;
        if (strncmp(abs_path, clean_abs, clean_abs_len) == 0 && abs_path[clean_abs_len] == '/') {
            rel_path = abs_path + clean_abs_len + 1;
        }

        if (rel_path_is_hidden_from_reporting(rel_path)) {
            continue;
        }
        report_skipped_dirs[report_skipped_count++] = abs_path;
    }

    /* Combine reportable extra files and reportable skipped directories for display */
    int combined_extra_capacity = report_extra_count + report_skipped_count;
    int combined_extra_count = 0;
    const char **combined_extra = arena_malloc((combined_extra_capacity > 0 ? combined_extra_capacity : 1) * sizeof(char*));

    for (int i = 0; i < report_extra_count; i++) {
        combined_extra[combined_extra_count++] = report_extra_abs[i];
    }
    for (int i = 0; i < report_skipped_count; i++) {
        combined_extra[combined_extra_count++] = report_skipped_dirs[i];
    }

    if (combined_extra_count > 0) {
        user_print_separator_line("EXTRA FILES");
        user_message("\n");

        int file_count = report_extra_count;
        int dir_count = report_skipped_count;
        ...
        if (git_exclude_extras && (report_extra_count > 0 || report_skipped_count > 0)) {
            ...
            int already_excluded_count = 0;
            if (gitattr_exists) {
                already_excluded_count = count_existing_gitattributes_exclusions_for_extras(
                    clean_abs,
                    (const char **)report_extra_abs,
                    report_extra_count,
                    (const char **)report_skipped_dirs,
                    report_skipped_count
                );
            }

            user_print_gitattributes_preview_for_extras(
                clean_abs,
                (const char **)report_extra_abs,
                report_extra_count,
                (const char **)report_skipped_dirs,
            ...
            if (gitattr_exists) {
                if (!prompt_yes_no_default_yes_line("Would you like me to update the .gitattributes file? [Y/n] ")) {
                    user_message("Aborted.\n");
                    return 0;
                }
            } else {
                if (!prompt_yes_no_default_yes_line("Would you like me to write the .gitattributes file? [Y/n] ")) {
                    user_message("Aborted.\n");
                    return 0;
                }
            }
            ...
            int result = write_gitattributes_for_extras(
                clean_abs,
                (const char **)report_extra_abs,
                report_extra_count,
                (const char **)report_skipped_dirs,
            ...
            if (result == 0) {
                ...
                user_message("Added exclusions for %d %s and %d %s.\n\n",
                             report_extra_count, en_plural_s((long)report_extra_count, "file", "files"),
                             report_skipped_count, en_plural_s((long)report_skipped_count, "directory", "directories"));
            } else {
